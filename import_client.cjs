// KS&P Limited — Import Script (no user creation)
...
async function safeInsert(table, payload, label) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (col) { console.log(`  ⚠ Removing unknown column "${col}" and retrying`); delete data[col]; continue; }
...
async function main() {
  ...
  // STEP 1: Safety test
  console.log("STEP 1: Safety test insert…");
  const tp = await safeInsert("projects", {
    name:"__TEST_IMPORT__", client:CLIENT_NAME, color:"#ccc",
    description:"", assigned_users:[], deadline:"2099-01-01",
  }, "test project");
  if (!tp) { console.log("❌ ABORTED — no data changed."); process.exit(1); }

  const tt = await safeInsert("tasks", {
    project_id:tp.id, title:"__TEST_TASK__", status:"Not Yet Started",
  ...
  if (!tt) {
    ...
    console.log("❌ ABORTED — test task failed. No data changed.");
    process.exit(1);
  }
  ...
  // STEP 2: Delete existing KS&P data
  console.log("STEP 2: Removing existing KS&P projects & tasks…");
  ...
  console.log(`  Found ${toDelete.length} existing project(s)`);
  ...
  // STEP 3: Insert projects + tasks
  console.log("\nSTEP 3: Inserting projects and tasks…");
  const grouped = {};
  ...
  for (const [projName, tasks] of Object.entries(grouped)) {
    ...
    // assigned_users: unique names from this project (stored as text)
    const names = [...new Set(tasks.flatMap(t => [t.assignee, t.detailer, t.checker].filter(Boolean)))];
    ...
    const proj = await safeInsert("projects", {
      ...
      description:"", assigned_users:names, deadline,
    }, `project "${projName}"`);
