...
function WarRoomPage({me,projects,users}){
  ...
  useEffect(()=>{
    ...
    function appendNew(rows){
      ...
      setMessages(prev=>{
        ...
        setLastMsgs(p=>({...p,[activePid]:fresh[fresh.length-1]}));
    ...
    // Polling fallback — every 5s fetch messages newer than last known
    poll=setInterval(async()=>{
      const since=lastMsgAtRef.current;
      if(!since)return;
      const{data}=await supabase.from("war_room_messages").select("*")
        .eq("project_id",activePid).gt("created_at",since)
  ...
  async function send(){
    ...
    const{data:inserted}=await supabase.from("war_room_messages")
      ...
      .select().single();
    if(inserted)appendNew([inserted]); // show immediately without waiting for poll
    if(mentions.length){
  ...
  return(
    ...
    {dashStatModal&&(()=>{
      ...
      const modalData={
        ...
        completed:{title:"✅ Completed Tasks",color:C.green,items:completedTasks.map(t=>{const pj=projects.find(p=>p.id===t