...
function WarRoomPage({me,projects,users}){
  ...
  function canSendInRoom(cl){
    if(!cl)return false;
    return true; // everyone can send in all client rooms
  }
  ...
  // Load history preview for a client
  useEffect(()=>{
    if(!historyClientId){setHistoryMsgs([]);return;}
    setHistoryLoading(true);
    supabase.from("war_room_messages").select("*")
      .eq("client_id",historyClientId)
      .order("created_at",{ascending:false})
      .limit(20)
      .then(({data})=>{
        setHistoryMsgs((data||[]).reverse());
  ...
  async function send(){
    ...
    const optimistic={id:tempId,client_id:capturedCid,author:me.username,author_name:me.name,body:msgBody,mentions,video_url:null,created_at:new Date().toISOString(),_pending:true};
    ...
    // Update optimistic bubble with media url if any
    ...
    const{data:inserted}=await supabase.from("war_room_messages")
      .insert({client_id:capturedCid,author:me.username,author_name:me.name,body:msgBody,mentions,video_url})
      .select().single();

    // Replace optimistic bubble with real DB row (gets real id + created_at)
    if(inserted){
      lastMsgAtRef.current=inserted.created_at;
      setMessages(prev=>prev.map(m=>m.id===tempId?inserted:m));
      setLastMsgs(p=>({...p,[capturedCid]:inserted}));
    }else{
      // DB failed — mark as failed but keep visible
    ...
    if(isAnnouncement){
      // Announcement → notify ALL users except sender
      const rows=users.filter(u=>u.username!==me.username).map(u=>({
        user_id:u.id,type:"announcement",
        title:`📢 ${me.name}: ${clientDisplayN}`,
        description:snippet,
        entity_type:"war_room",entity_id:capturedCid,created_by:me.username
      }));
      if(rows.length)await supabase.from("notifications").insert(rows);
      setIsAnnouncement(false);
    }else{
      // Regular message → notify client user + Admins + Managers (not sender)
      const recipients=users.filter(u=>
        u.username!==me.username&&
        (u.username===capturedCid||["Admin","Manager"].includes(u.role))
      );
      if(recipients.length){
        const rows=recipients.map(u=>({
          user_id:u.id,type:"war_room_message",
          title:`💬 ${me.name} in ${clientDisplayN}`,
          description:snippet,
          entity_type:"war_room",entity_id:capturedCid,created_by:me.username
        }));
        await supabase.from("notifications").insert(rows);
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:C.card,border:`1px solid ${C.border}`,borderTop:"none",borderRadius:isMobile?"0 0 10px 10px":"0 0 14px 14px",overflow:"hidden",minHeight:0}}>
        ...
        {canSend?(
          <div style={{position:"relative"}}>
            ...
            {isAnnouncement&&(
              <div style={{padding:"6px 14px",background:"#f59e0b22",borderTop:`1px solid #f59e0b55`,display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:13}}>📢</span>
                <span style={{fontSize:12,color:"#f59e0b",fontWeight:700}}>Announcement mode — this message will notify everyone</span>
                <button onClick={()=>setIsAnnouncement(false)} style={{marginLeft:"auto",background:"transparent",border:"none",color:"#f59e0b",cursor:"pointer",fontSize:13,fontWeight:700}}>✕</button>
              </div>
            )}
            <div style={{padding:isMobile?"8px":"10px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:isMobile?6:8,alignItems:"center",background:C.card}}>
              ...
              <button onClick={()=>setIsAnnouncement(a=>!a)} title={isAnnouncement?"Cancel Announcement":"Send as Announcement (notifies everyone)"}
                style={{flexShrink:0,width:isMobile?34:36,height:isMobile?34:36,borderRadius:"50%",background:isAnnouncement?"#f59e0b22":C.surface,border:`1px solid ${isAnnouncement?"#f59e0b":C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?15:16,transition:"all .15s"}}>
                📢
              </button>
              <button onClick={send} disabled={sending||uploading||(!input.trim()&&!mediaFile)}
                style={{flexShrink:0,background:isAnnouncement?"#f59e0b":C.teal,border:"none",borderRadius:isMobile?"50%":"10px",width:isMobile?36:undefined,height:isMobile?36:undefined,padding:isMobile?0:"9px 18px",color:"#fff",fontSize:isMobile?18:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",opacity:(sending||uploading||(!input.trim()&&!mediaFile))?0.5:1,transition:"all .15s",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                {isMobile?(uploading?"⬆":"➤"):(isAnnouncement?"📢 Announce":"Send")}
              </button>
            </div>
          </div>
        ):(
            ...
            {overdueTasks.length>0&&(<>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(320px,100%),1fr))",gap:18,marginBottom:28}}>
                {overdueTasks.map(t=>{
                  ...
                  const team=[t.assignee,t.detailer,t.checker].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);
                  return(
                    <div key={t.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:20,cursor:"pointer",borderTop:`4px solid ${C.red}`,transi