...
{isMobile&&uMenu&&(
    ...
    {/* ── Mobile bottom nav ── */}
    <nav className="rds-bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,background:C.card,borderTop:`1px solid ${C.border}`,zIndex:200,paddingBottom:"env(safe-area-inset-bottom,0px)",overflowX:"auto",WebkitOverflowScrolling:"touch",alignItems:"stretch"}}>
      {navs.map(([k,ico,lbl])=>{const badge=navBadges[k]||0;const active=view===k;return(
        <button key={k} onClick={()=>{navTo(k,k==='list'?activePid:null);setSO(false);if(badge>0)setNavBadges(prev=>({...prev,[k]:0}));}}
          style={{flex:"1 0 auto",minWidth:56,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:"8px 4px",position:"relative",color:active?C.accent:C.t3,fontFamily:"inherit",transition:"color .15s"}}>
      ...
      <button onClick={()=>sMenu(v=>!v)}
        style={{flex:"1 0 auto",minWidth:56,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:"8px 4px",position:"relative",color:uMenu?C.accent:C.t3,fontFamily:"inherit",transition:"color .15s"}}>
