...
export default function App(){
  ...
  // Load nav badge counts (unread notifications by type)
  useEffect(()=>{
    if(!me)return;
    async function loadBadges(){
      const{data}=await supabase.from("notifications")
        .select("type,is_read").eq("user_id",me.id).eq("is_read",false);
      const warroom=(data||[]).filter(n=>n.type==="war_room_message"||n.type==="mention").length;
      const announcements=(data||[]).filter(n=>n.type==="announcement").length;
      setNavBadges({warroom,announcements});
    }
    loadBadges();
    // Realtime update
    const ch=supabase.channel("nav-badges-"+me.id)
      .on("postgres_changes",{event:"*",schema:"public",table:"notifications",filter:`user_id=eq.${me.id}`},()=>loadBadges())
      .subscribe();
    return()=>supabase.removeChannel(ch);
  },[me]);
