...
function WarRoomPage({me,projects,users}){
  ...
  // `projects` is already accessibleProjects (pre-filtered per role in parent)
  const visibleClients=clients.filter(cl=>{
    ...
    return projects.some(p=>p.client===cl.client_name);
  });
  ...
  function canSendInRoom(cl){
    ...
    if(["Admin","Manager"].includes(me.role))return true;
    if(me.role==="Client")return cl.username===me.username;
    // TL / Rebar / Tekla: can send if any of their accessible projects belongs to this client
    return projects.some(p=>p.client===cl.client_name);
  }
  ...
  async function send(){
    ...
    if(mentions.length){
      const rows=users.filter(u=>mentions.includes(u.username)&&u.username!==me.username).map(u=>({user_id:u.id,type:"mention",title:`💬 ${me.name} mentioned you in War Room`,description:msgBody.slice(0,100),entity_type:"war_room",entity_id:capturedCid,created_by:me.username}));
      if(rows.length)await supabase.from("notifications").insert(rows);
    }
    setSending(false);
  }
