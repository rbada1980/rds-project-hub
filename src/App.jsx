...
export default function App(){
  ...
  return(
    ...
    <div style={{height:"100vh",width:"100vw",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif",color:C.t1,display:"flex",overflow:"hidden",position:"fixed",top:0,left:0}}>
      ...
      {isMobile&&<nav className="rds-bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,height:60,zIndex:200,background:C.card,borderTop:`1px solid ${C.border}`,alignItems:"stretch",overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        {navs.map(([k,ico,lbl])=>{const badge=navBadges[k]||0;const active=view===k;return(<button key={k} onClick={()=>{navTo(k,k==='list'?activePid:null);if(badge>0)setNavBadges(prev=>({...prev,[k]:0}));}} style={{flex:"1 0 auto",minWidth:54,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:"6px 4px",position:"relative",color:active?C.accent:C.t3,transition:"color .15s"}}><span style={{fontSize:20,lineHeight:1}}>{ico}</span><span style={{fontSize:9,fontWeight:active?700:500,letterSpacing:".02em",whiteSpace:"nowrap"}}>{lbl}</span>{badge>0&&<span style={{position:"absolute",top:4,right:"calc(50% - 18px)",background:C.red,color:"#fff",borderRadius:"50%",width:16,height:16,fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>{badge>9?"9+":badge}</span>}{active&&<span style={{position:"absolute",top:0,left:"20%",right:"20%",height:2,background:C.accent,borderRadius:"0 0 2px 2px"}}/>}</button>);})}
      </nav>}
